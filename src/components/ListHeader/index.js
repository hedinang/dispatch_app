    ...
    <TableHead className={classTableHead}>
      <TableRow>
        {fields.map(
          (field) =>
            !field.hidden && (
              <TableCell key={field.id} sortDirection={orderBy === field.id ? (desc ? 'desc' : 'asc') : false} className={classTableCell} style={{minWidth: field.minWidth || 'auto', color: field.color || 'inherit'}}>
                {!field.isDisableSort ? (
                  <TableSortLabel
